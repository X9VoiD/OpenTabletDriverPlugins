...
namespace OpenTabletDriverPlugins
{
    ...
    {
        public AdvancedAntiSmoothing()
        {
            TabletDriverPlugin.Log.Write("AdvAntiSmooth", "Filter constructed.");
        }
        ...
        public virtual Point Filter(Point point)
        {
            ...
            //AddTimeDeltaSample(timeDelta);
            //float TimeDeltaAvg = Average(_timeDeltaAvg);
        ...
        private void AddTimeDeltaSample(float timeDelta)
        {
            _timeDeltaAvg.AddLast(timeDelta);
            if (_timeDeltaAvg.Count > Samples)
            {
                for (int i = _timeDeltaAvg.Count; i > Samples; --i)
                    _timeDeltaAvg.RemoveFirst();
            }
        }

        private float Average(LinkedList<float> samples)
        {
            float weight = 2, sumOfWeights = 0;
            float avg = 0;
        ...
        private LinkedList<float> _timeDeltaAvg = new LinkedList<float>();
